...
class Bundler extends EventEmitter {
  ...
  bundle() {
    ...
    return _asyncToGenerator(function* () {
      ...
      _this2.emit('buildStart', _this2.entryFiles);
  ...
  resolveAsset(name, parent) {
    ...
    return _asyncToGenerator(function* () {
      ...
      return _this5.getLoadedAsset(path);
    })();
  }
  ...
  watch(path, asset) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (!_this6.watchedAssets.has(path)) {
        _this6.watcher.watch(path);
        _this6.watchedAssets.set(path, new Set());
      }
      _this6.watchedAssets.get(path).add(asset);
    })();
  }
  ...
  unwatch(path, asset) {
    ...
    return _asyncToGenerator(function* () {
      ...
      let watched = _this7.watchedAssets.get(path);
      watched.delete(asset);

      if (watched.size === 0) {
        _this7.watchedAssets.delete(path);
        _this7.watcher.unwatch(path);
      }
    })();
  }
  ...
  resolveDep(asset, dep, install = true) {
    ...
    return _asyncToGenerator(function* () {
      try {
        if (dep.resolved) {
          return _this8.getLoadedAsset(dep.resolved);
        }

        return yield _this8.resolveAsset(dep.name, asset.name);
      } catch (err) {
        ...
        if (err.code === 'MODULE_NOT_FOUND') {
          ...
          if (!isLocalFile && !fromNodeModules && _this8.options.autoinstall && install) {
            return yield _this8.installDep(asset, dep);
          }
          ...
          yield _this8.throwDepError(asset, dep, err);
        }
  ...
  installDep(asset, dep) {
    ...
    return _asyncToGenerator(function* () {
      ...
      let resolved = yield _this9.resolver.resolveModule(dep.name, asset.name);
      ...
      return yield _this9.resolveDep(asset, dep, false);
    })();
  }
  ...
  processAsset(asset, isRebuild) {
    ...
    return _asyncToGenerator(function* () {
      if (isRebuild) {
        asset.invalidate();
        if (_this10.cache) {
          _this10.cache.invalidate(asset.name);
        }
      }

      yield _this10.loadAsset(asset);
    })();
  }
  ...
  loadAsset(asset) {
    ...
    return _asyncToGenerator(function* () {
      ...
      let processed = _this11.cache && (yield _this11.cache.read(asset.name));
      let cacheMiss = false;
      if (!processed || asset.shouldInvalidate(processed.cacheData)) {
        processed = yield _this11.farm.run(asset.name, asset.id);
      ...
      if (_this11.delegate.getImplicitDependencies) {
        let implicitDeps = yield _this11.delegate.getImplicitDependencies(asset);
        if (implicitDeps) {
      ...
      let assetDeps = yield Promise.all(dependencies.map((() => {
        var _ref2 = _asyncToGenerator(function* (dep) {
          if (dep.includedInParent) {
            ...
            _this11.watch(dep.name, asset);
          } else {
            dep.parent = asset.name;
            let assetDep = yield _this11.resolveDep(asset, dep);
            if (assetDep) {
              yield _this11.loadAsset(assetDep);
            }
      ...
      if (_this11.cache && cacheMiss) {
        _this11.cache.write(asset.name, processed);
      }
    })();
  }
  ...
  *findOrphanAssets() {
  ...
  onChange(path) {
    ...
    return _asyncToGenerator(function* () {
      let assets = _this12.watchedAssets.get(path);
      ...
      try {
        for (var _iterator11 = assets[Symbol.iterator](), _step11; !(_iteratorNormalCompletion11 = (_step11 = _iterator11.next()).done); _iteratorNormalCompletion11 = true) {
          ...
          _this12.buildQueue.add(asset, true);
        }
      } catch (err) {
      ...
      clearTimeout(_this12.rebuildTimeout);

      _this12.rebuildTimeout = setTimeout(_asyncToGenerator(function* () {
        yield _this12.bundle();
      }), 100);
    })();
  }
  ...
  serve(port = 1234, https = false) {
    ...
    return _asyncToGenerator(function* () {
      _this13.server = yield Server.serve(_this13, port, https);
      try {
        yield _this13.bundle();
      } catch (e) {
        // ignore: server can still work with errored bundler
      }
      return _this13.server;
    })();
  }
}
